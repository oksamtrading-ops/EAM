/**
 * rationalize_application — LLM-as-judge eval.
 *
 * For each fixture under `fixtures/rationalize-application/`, seeds
 * an ephemeral workspace, runs the sub-agent 3× (production temp,
 * absorbs sampling noise), scores each run with Opus 4.6 against
 * the rubric at `rubrics/rationalizeApp.v1.ts`, and writes the
 * archive slice to `results/last-run.json` (schema v2).
 *
 * Smoke assertion only (avgScore > 0). Threshold tightens after
 * 5–10 runs reveal the empirical baseline.
 *
...
import {
  RATIONALIZE_APP_RUBRIC,
  RATIONALIZE_APP_RUBRIC_VERSION,
} from "./rubrics/rationalizeApp.v1";
import {
  diffSlice,
  loadPreviousRun,
  writeArchiveSlice,
...
import { RATIONALIZE_APP_FIXTURES } from "./fixtures/rationalize-application";

const RUN = process.env.RUN_EVALS === "1";
const describeMaybe = RUN ? describe : describe.skip;

const SUB_AGENT = "rationalizeApp";
...
describeMaybe("rationalize_application — LLM judge", () => {
  const fixtureResults: LocalFixtureResult[] = [];
  const previous = loadPreviousRun();

  for (const fixture of RATIONALIZE_APP_FIXTURES) {
    it(
      ...
      async () => {
        ...
        try {
          const agentRuns = await Promise.all(
            [1, 2, 3].map(() =>
              runSubAgent("rationalize_application", seed.input, {
  ...
  it("write archive slice + total cost", () => {
    ...
    writeArchiveSlice(SUB_AGENT, {
      rubricVersion: RATIONALIZE_APP_RUBRIC_VERSION,
