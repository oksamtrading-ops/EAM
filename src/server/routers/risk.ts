...
export const riskRouter = router({
  ...
  runAutoScan: workspaceProcedure.mutation(async ({ ctx }) => {
    ...
    // Data governance risks — entities missing steward / owner / custodian /
    // golden source / classification. Findings come from the shared service
    // so that Risk and the Dashboard DataKpiStrip can never drift.
    const findings = await computeDataFindings(ctx.db, ctx.workspaceId);
...
type Likelihood = "RARE" | "LOW" | "MEDIUM" | "HIGH";
type Impact = "LOW" | "MEDIUM" | "HIGH" | "CRITICAL";

function severityForFinding(f: DataFinding): {
  likelihood: Likelihood;
  impact: Impact;
  title: string;
  description: string;
} {
  const where = `"${f.entityName}" (${f.domainName})`;
  const sensitive = f.regulatoryTags.length > 0 ? ` — tagged ${f.regulatoryTags.join(", ")}` : "";

  const map: Record<
    DataFindingKind,
    { likelihood: Likelihood; impact: Impact; title: string; description: string }
  > = {
    SENSITIVE_NO_STEWARD: {
      likelihood: "HIGH",
      impact: "HIGH",
      title: `Sensitive data without a steward: ${f.entityName}`,
      description: `${where} is classified ${f.classification}${sensitive} but has no assigned data steward. Regulatory response and incident triage will be blocked.`,
    },
    NO_GOLDEN: {
      likelihood: "HIGH",
      impact: "HIGH",
      title: `No golden source for sensitive entity: ${f.entityName}`,
      description: `${where} is sensitive (${f.classification})${sensitive} but has no golden source application declared. Conflicting copies across apps will cause data integrity issues.`,
    },
    NO_OWNER: {
      likelihood: "MEDIUM",
      impact: "MEDIUM",
      title: `No business owner: ${f.entityName}`,
      description: `${where} has no business owner. Decisions about access, retention, and sharing have no accountable executive.`,
    },
    NO_STEWARD: {
      likelihood: "MEDIUM",
      impact: "MEDIUM",
      title: `No data steward: ${f.entityName}`,
      description: `${where} has no data steward. Data quality and definitional questions have no one to answer them.`,
    },
    NO_CUSTODIAN: {
      likelihood: "LOW",
      impact: "MEDIUM",
      title: `No technical custodian: ${f.entityName}`,
      description: `${where} has no technical custodian. No one is on the hook for backups, access controls, or schema changes.`,
    },
    UNCLASSIFIED: {
      likelihood: "MEDIUM",
      impact: "LOW",
